...
from .base import CommandWrapper, CommandResult
...
class PCT(CommandWrapper):
    ...
    @staticmethod
    def create_cmd(container_id: str, template_path: str, hostname: str,
                   memory: int, swap: int, cores: int, ip_address: str, gateway: str,
                   bridge: str, storage: str, rootfs_size: int, unprivileged: bool = True,
                   ostype: str = "ubuntu", arch: str = "amd64") -> str:
        """Generate command to create a container"""
        unpriv_flag = "1" if unprivileged else "0"
        return (
            f"pct create {container_id} {template_path} "
            f"--hostname {hostname} "
            f"--memory {memory} --swap {swap} --cores {cores} "
            f"--net0 name=eth0,bridge={bridge},firewall=1,gw={gateway},ip={ip_address}/24,ip6=dhcp,type=veth "
            f"--rootfs {storage}:{rootfs_size} --unprivileged {unpriv_flag} "
            f"--ostype {ostype} --arch {arch} 2>&1"
        )
    ...
    @staticmethod
    def set_features_cmd(container_id: str, nesting: bool = True, 
                        keyctl: bool = True, fuse: bool = True) -> str:
    ...
    def exists_check_cmd(container_id: str) -> str:
        ...
        return f"test -f /etc/pve/lxc/{container_id}.conf && echo exists || echo missing"
