...
def create_container(container_cfg, cfg: LabConfig, step_num, total_steps):
    ...
    logger.info(
        "[%s/%s] Creating container '%s' (type: %s)...",
        step_num,
        total_steps,
        container_name,
        container_type,
...
def create_template(template_cfg, cfg: LabConfig, step_num, total_steps):
    ...
    logger.info(
        "[%s/%s] Creating template '%s' (type: %s)...",
        step_num,
        total_steps,
        template_name,
        template_type,
...
def cmd_status():
    ...
    result = ssh_exec(
        cfg.proxmox_host, list_cmd, check=False, capture_output=True, cfg=cfg
    )
    if result:
        logger.info(result)
    else:
    ...
    docker_cmd = (
        docker_path.strip() if docker_path and docker_path.strip() else "docker"
    )
...
def main():
    ...
    deploy_parser = subparsers.add_parser(
        "deploy", help="Deploy complete lab: apt-cache, templates, and Docker Swarm"
    )
    deploy_parser.set_defaults(func=cmd_deploy)

    cleanup_parser = subparsers.add_parser(
        "cleanup", help="Remove all containers and templates"
    )
