...
from .common import (container_exists, destroy_container, setup_ssh_key, ssh_exec, wait_for_container)
from .config import ContainerConfig, ContainerResources, LabConfig
from .template import get_base_template
# Get logger for this module
logger = logging.getLogger(__name__)

def _check_container_exists(proxmox_host, _container_id, list_check_cmd, exists_check_cmd, cfg):
    """Check if container exists by querying list and config."""
    list_check = ssh_exec(proxmox_host, list_check_cmd, check=False, timeout=10, cfg=cfg)
    config_check = ssh_exec(proxmox_host, exists_check_cmd, check=False, timeout=10, cfg=cfg)
    return (list_check and "exists" in list_check) or (config_check and "exists" in config_check)
...
def _try_base_template_fallback(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    if not using_custom_template:
        return False, None
    logger.warning("Template %s failed, trying with base template...", template_name)
    ...
    fallback_result = ssh_exec(proxmox_host, fallback_create_cmd, check=False, cfg=cfg)
    time.sleep(3)
    list_check = ssh_exec(proxmox_host, list_check_cmd, check=False, timeout=10, cfg=cfg)
    if list_check and "exists" in list_check:
        logger.info("Container %s created successfully with base template", container_id)
        return True, None
    logger.error("Container %s creation failed even with base template", container_id)
    error_msg = fallback_result[-500:] if fallback_result else "Unknown error"
...
def setup_container_base(  # pylint: disable=too-many-locals,too-many-return-statements,too-many-branches,too-many-statements
    ...
    template_valid = ssh_exec(proxmox_host, template_validate_cmd, check=False, cfg=cfg)
    ...
    create_result = ssh_exec(proxmox_host, create_cmd, check=False, cfg=cfg)
    ...
    exists_check_cmd = PCT().container_id(container_id).exists_check()
    container_exists_flag = _check_container_exists(proxmox_host, container_id, list_check_cmd, exists_check_cmd, cfg)
    if not container_exists_flag:
        ...
        if create_result and "tar:" in create_result:
            ...
            retry_result = ssh_exec(proxmox_host, create_cmd, check=False, cfg=cfg)
            ...
            if not container_exists_flag and retry_result and "tar:" in retry_result:
                ...
                time.sleep(5)
                ssh_exec(proxmox_host, f"pct destroy {container_id} --force 2>/dev/null || true", check=False, cfg=cfg)
                time.sleep(3)
                ...
                if template_check and "template missing" not in template_check:
                    retry2_result = ssh_exec(proxmox_host, create_cmd, check=False, cfg=cfg)
                    time.sleep(5)
                    container_exists_flag = _check_container_exists(
                        proxmox_host, container_id, list_check_cmd, exists_check_cmd, cfg
                    )
            if not container_exists_flag:
                ...
                if list_check and "exists" in list_check:
                    logger.warning("Container %s exists despite tar errors, continuing...", container_id)
                    ...
                    if success:
                        ...
                        error_msg = (
                            error_msg or retry_result[-500:]
                            if retry_result
                            else create_result[-500:] if create_result else "Unknown error"
                        )
            ...
            if "ubuntu-25.04-template" in template_path or "ubuntu-tmpl" in template_path:
                logger.warning("Custom template failed, trying with original " "ubuntu-24.10-standard template...")
                ...
                if list_check and "exists" in list_check:
                    logger.info("Container %s created successfully with original template", container_id)
                    container_exists_flag = True
                else:
                    ...
                    if final_check and "exists" in final_check:
                        ...
                        logger.error("Container %s creation failed even with original template", container_id)
                        error_msg = (
                            fallback_result[-500:]
                            if fallback_result
                            else retry_result[-500:] if retry_result else create_result[-500:]
                        )
                        logger.error("Error output: %s", error_msg)
                        return False
            else:
                # Check if maybe container was created anyway
                logger.warning("Checking if container %s exists despite errors...", container_id)
                time.sleep(2)
                final_check = ssh_exec(
                    proxmox_host,
                    list_check_cmd,
                ...
                if final_check and "exists" in final_check:
                    ...
                    logger.error("Container %s creation failed after retry", container_id)
                    error_msg = retry_result[-500:] if retry_result else create_result[-500:]
                    logger.error("Error output: %s", error_msg)
                    return False
    ...
    if start_result.has_error:
        ...
        if privileged:
            ...
            config_cmd = PCT().container_id(container_id).config()
            config_output = ssh_exec(proxmox_host, config_cmd, check=False, cfg=cfg)
            if config_output:
    ...
    for attempt in range(1, max_start_attempts + 1):
        ...
        status_output = ssh_exec(proxmox_host, status_cmd, check=False, cfg=cfg)
        ...
        if attempt < max_start_attempts:
            ...
            if status_output and "stopped" in status_output:
                logger.info("Container %s stopped, trying to start again...", container_id)
            ...
            if privileged:
                ...
                error_output = ssh_exec(proxmox_host, start_cmd, check=False, cfg=cfg)
                error_result = CommandWrapper.parse_result(error_output)
                if error_result.has_error:
                    logger.error("Start error: %s - %s", error_result.error_type.value, error_result.error_message)
            return False
    # Configure network
    ...
    if not lxc_service.connect():
        logger.error("Failed to connect to Proxmox host for network configuration")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False
    try:
        pct_service = PCTService(lxc_service)
        network_result, _ = pct_service.execute(container_id, network_cmd, timeout=10)
    ...
    lxc_service = LXCService(proxmox_host, cfg.ssh)
    if not lxc_service.connect():
        logger.error("Failed to connect to Proxmox host for user setup")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False
    try:
        pct_service = PCTService(lxc_service)
        user_result, _ = pct_service.execute(container_id, user_cmd, timeout=30)
        if not user_result or ("error" in user_result.lower() and "User exists" not in user_result):
            logger.error("Failed to setup user: %s", user_result[-300:] if user_result else "No output")
            destroy_container(proxmox_host, container_id, cfg=cfg)
            return False
    finally:
        lxc_service.disconnect()
    ...
    remaining_dns_cmd = " && ".join([f"echo 'nameserver {dns}' >> /etc/resolv.conf" for dns in dns_servers[1:]])
    dns_setup_cmd = f"echo 'nameserver {dns_servers[0]}' > /etc/resolv.conf"
    if remaining_dns_cmd:
        dns_setup_cmd = f"{dns_setup_cmd} && {remaining_dns_cmd}"
    dns_setup_cmd = f"{dns_setup_cmd} 2>&1"
    lxc_service = LXCService(proxmox_host, cfg.ssh)
    if not lxc_service.connect():
        logger.error("Failed to connect to Proxmox host for DNS configuration")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False
    try:
        pct_service = PCTService(lxc_service)
        dns_result, _ = pct_service.execute(container_id, dns_setup_cmd, timeout=30)
    ...
    lxc_service = LXCService(proxmox_host, cfg.ssh)
    if not lxc_service.connect():
        logger.error("Failed to connect to Proxmox host for apt sources fix")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False
    try:
        pct_service = PCTService(lxc_service)
        sources_output, _ = pct_service.execute(container_id, fix_sources_cmd)
    ...
    if configure_proxy:
        ...
        if apt_cache_containers:
            ...
            lxc_service = LXCService(proxmox_host, cfg.ssh)
            if not lxc_service.connect():
                logger.error("Failed to connect to Proxmox host for apt cache configuration")
                destroy_container(proxmox_host, container_id, cfg=cfg)
                return False
            try:
                pct_service = PCTService(lxc_service)
                apt_cache_result, _ = pct_service.execute(container_id, proxy_cmd)
