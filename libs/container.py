...
from .common import ssh_exec, pct_exec, destroy_container, wait_for_container, setup_ssh_key, container_exists
from .template import get_base_template
from .config import LabConfig, ContainerConfig
from cli import PCT, Apt, CommandWrapper
...
def setup_container_base(container_cfg: ContainerConfig, cfg: LabConfig, privileged=False):
    ...
    if not resources:
        # Default fallback
        from .config import ContainerResources
    ...
    create_result = ssh_exec(proxmox_host, create_cmd, check=False, capture_output=True, cfg=cfg)
    ...
    time.sleep(1)
    list_check_cmd = f"pct list 2>/dev/null | grep -E '^{container_id}\\s' && echo exists || echo missing"
    list_check = ssh_exec(proxmox_host, list_check_cmd, check=False, capture_output=True, timeout=10, cfg=cfg)
    
    # Also check config file
    exists_check_cmd = PCT.exists_check_cmd(container_id)
    config_check = ssh_exec(proxmox_host, exists_check_cmd, check=False, capture_output=True, timeout=10, cfg=cfg)
    
    container_exists_flag = (list_check and "exists" in list_check) or (config_check and "exists" in config_check)
    ...
    if not container_exists_flag:
        ...
        if create_result and "tar:" in create_result:
            ...
            retry_result = ssh_exec(proxmox_host, create_cmd, check=False, capture_output=True, cfg=cfg)
            ...
            if not container_exists_flag:
                ...
                if list_check and "exists" in list_check:
                    logger.warning(f"Container {container_id} exists despite tar errors, continuing...")
                    container_exists_flag = True
                else:
                    # Try one more time with base template as fallback if custom template fails
                    # Check if we're using a custom template (not the base ubuntu template)
                    using_custom_template = template_name and template_name != 'ubuntu-tmpl' and 'ubuntu-25.04-template' not in template_path and 'ubuntu-24.10-standard' not in template_path
                    if using_custom_template:
                        logger.warning(f"Template {template_name} failed, trying with base template...")
                        base_template = get_base_template(proxmox_host, cfg)
                        ...
                        fallback_result = ssh_exec(proxmox_host, fallback_create_cmd, check=False, capture_output=True, cfg=cfg)
                        time.sleep(3)
                        list_check = ssh_exec(proxmox_host, list_check_cmd, check=False, capture_output=True, timeout=10, cfg=cfg)
                        if list_check and "exists" in list_check:
                            logger.info(f"Container {container_id} created successfully with base template")
                            container_exists_flag = True
                        else:
                            logger.error(f"Container {container_id} creation failed even with base template")
                            logger.error(f"Error output: {fallback_result[-500:] if fallback_result else retry_result[-500:] if retry_result else create_result[-500:]}")
                            return False
                    else:
                        # Already using base template or template path issue
                        # Try with the original ubuntu-24.10-standard template as last resort
                        if 'ubuntu-25.04-template' in template_path or 'ubuntu-tmpl' in template_path:
                            logger.warning(f"Custom template failed, trying with original ubuntu-24.10-standard template...")
                            original_template = "ubuntu-24.10-standard_24.10-1_amd64.tar.zst"
                            original_template_path = f"{cfg.proxmox_template_dir}/{original_template}"
                            time.sleep(2)
                            ssh_exec(proxmox_host, f"pct destroy {container_id} 2>/dev/null || true", check=False, cfg=cfg)
                            time.sleep(1)
                            fallback_create_cmd = PCT.create_cmd(
                            ...
                            list_check = ssh_exec(proxmox_host, list_check_cmd, check=False, capture_output=True, timeout=10, cfg=cfg)
                            if list_check and "exists" in list_check:
                                logger.info(f"Container {container_id} created successfully with original template")
                                container_exists_flag = True
                            else:
                                # Check one more time if container exists despite errors
                                logger.warning(f"Checking if container {container_id} exists despite errors...")
                                time.sleep(2)
                                final_check = ssh_exec(proxmox_host, list_check_cmd, check=False, capture_output=True, timeout=10, cfg=cfg)
                                if final_check and "exists" in final_check:
                                    logger.warning(f"Container {container_id} exists despite tar errors, continuing...")
                                    container_exists_flag = True
                                else:
                                    logger.error(f"Container {container_id} creation failed even with original template")
                                    logger.error(f"Error output: {fallback_result[-500:] if fallback_result else retry_result[-500:] if retry_result else create_result[-500:]}")
                                    return False
                        else:
                            ...
                            if final_check and "exists" in final_check:
                                ...
                                logger.error(f"Container {container_id} creation failed after retry")
                                logger.error(f"Error output: {retry_result[-500:] if retry_result else create_result[-500:]}")
                                return False
            else:
                logger.info(f"Container {container_id} created successfully on retry")
        else:
            # Other error - fail immediately
            logger.error(f"Container {container_id} creation failed")
            create_result_parsed = CommandWrapper.parse_result(create_result)
            if create_result_parsed.has_error:
                logger.error(f"Error: {create_result_parsed.error_type.value} - {create_result_parsed.error_message}")
            return False
    else:
        # Container exists - tar errors were non-fatal
        if create_result and "tar:" in create_result:
            logger.warning("Non-fatal tar errors during container creation (container was created successfully)")
    
    # Verify container exists
    from .common import container_exists as check_container_exists
    if not check_container_exists(proxmox_host, container_id, cfg=cfg):
        logger.error(f"Container {container_id} was not created")
        return False
    ...
    if start_result.has_error:
        logger.error(f"Failed to start container {container_id}: {start_result.error_type.value} - {start_result.error_message}")
        # Try to get more details about why it failed
        if privileged:
            logger.info("Checking container configuration for privileged container...")
            config_cmd = PCT.config_cmd(container_id)
            config_output = ssh_exec(proxmox_host, config_cmd, check=False, capture_output=True, cfg=cfg)
            if config_output:
                logger.info(f"Container config: {config_output[:500]}")
        return False
    ...
    for attempt in range(1, max_start_attempts + 1):
        ...
        status_output = ssh_exec(proxmox_host, status_cmd, check=False, capture_output=True, cfg=cfg)
        if PCT.parse_status_output(status_output, container_id):
            break
        if attempt < max_start_attempts:
            logger.info(f"Container {container_id} not running yet, waiting... (attempt {attempt}/{max_start_attempts})")
            time.sleep(5)
            # Try starting again if it stopped
            if status_output and "stopped" in status_output:
                logger.info(f"Container {container_id} stopped, trying to start again...")
                ssh_exec(proxmox_host, start_cmd, check=False, capture_output=True, cfg=cfg)
                time.sleep(5)
        else:
            # Last attempt failed - get more details
            logger.error(f"Container {container_id} is not running after {max_start_attempts} attempts. Status: {status_output}")
            # Get container logs if available
            if privileged:
                logger.info("Checking for startup errors in privileged container...")
                # Try to get any error messages
                error_output = ssh_exec(proxmox_host, start_cmd, check=False, capture_output=True, cfg=cfg)
                error_result = CommandWrapper.parse_result(error_output)
                if error_result.has_error:
                    logger.error(f"Start error: {error_result.error_type.value} - {error_result.error_message}")
            return False
    
    # Configure network
    logger.info("Configuring network...")
    network_result = pct_exec(proxmox_host, container_id,
         f"ip link set eth0 up && ip addr add {ip_address}/24 dev eth0 2>/dev/null || true && ip route add default via {gateway} dev eth0 2>/dev/null || true && sleep 2 2>&1",
         check=False, capture_output=True, timeout=10, cfg=cfg)
    if network_result and "error" in network_result.lower():
        logger.warning(f"Network configuration had issues: {network_result[-200:]}")
    ...
    if not user_result or ("error" in user_result.lower() and "User exists" not in user_result):
        logger.error(f"Failed to setup user: {user_result[-300:] if user_result else 'No output'}")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False
    ...
    dns_result = pct_exec(proxmox_host, container_id,
         f"echo 'nameserver {dns_servers[0]}' > /etc/resolv.conf && {dns_cmd.replace(dns_servers[0], '', 1).lstrip(' && ')} 2>&1",
         check=False, capture_output=True, timeout=30, cfg=cfg)
    if dns_result and "error" in dns_result.lower():
        logger.warning(f"DNS configuration had issues: {dns_result[-200:]}")
    ...
    if sources_result.has_error:
        logger.warning(f"Apt sources fix had issues: {sources_result.error_type.value} - {sources_result.error_message}")
