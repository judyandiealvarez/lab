...
def _ensure_user_exists(proxmox_host, container_id, cfg, username, sudo_group):
    exists_cmd = f"id -u {username} >/dev/null 2>&1"
    try:
        pct_exec(
            proxmox_host,
            container_id,
        ...
        create_cmd = f"useradd -m -s /bin/bash -G {sudo_group} {username} 2>&1"
...
def _cleanup_template_files(
    proxmox_host, container_id, cfg, default_user, extra_steps=None
):
    cleanup_commands = [
        (
        ...
        (
            f"Clear {default_user} history",
            FileOps.truncate_cmd(
                f"/home/{default_user}/.bash_history", suppress_errors=True
            ),
        ),
    ]
    if extra_steps:
        cleanup_commands.extend(extra_steps)
...
def _ensure_apparmor_stub(proxmox_host, container_id, cfg) -> bool:
    """Ensure AppArmor parser is diverted and stubbed to avoid profile reload failures."""
    stub_content = (
        "#!/bin/sh\n"
    ...
    steps = [
        ("Divert AppArmor parser", Dpkg.divert_cmd("/usr/sbin/apparmor_parser")),
        (
            "Write AppArmor parser stub",
            FileOps.write_cmd("/usr/sbin/apparmor_parser", stub_content),
        ),
        (
...
def create_template(template_cfg: TemplateConfig, cfg: LabConfig):  # pylint: disable=too-many-locals,too-many-return-statements,too-many-branches,too-many-statements
    ...
    features_output = ssh_exec(
        proxmox_host, features_cmd, capture_output=True, cfg=cfg
    )
    ...
    if not _ensure_apparmor_stub(proxmox_host, container_id, cfg):
        logger.warning(
            "AppArmor parser diversion failed; continuing but apt upgrades may fail"
        )
    ...
    if ssh_install_output is None:
        logger.error("Failed to install openssh-server in docker template container")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False
    ...
    if update_output is None:
        logger.error("apt update failed for docker template")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False

    logger.info("Installing base packages required for Docker provisioning...")
    prereq_packages = [
        "curl",
        "ca-certificates",
        "gnupg",
    ...
    prereq_output = run_apt_with_lock(
        proxmox_host,
        container_id,
        Apt.install_cmd(prereq_packages),
        cfg,
    )
    ...
    # Install Docker - rely on distro packages to avoid repo/proxy issues
    logger.info("Installing Docker from Ubuntu repository...")
    docker_packages = ["docker.io", "containerd"]
    docker_install_output = run_apt_with_lock(
        proxmox_host,
        container_id,
        Apt.install_cmd(docker_packages),
        cfg,
    )
    if docker_install_output is None:
        logger.error("Failed to install docker.io/containerd packages")
        destroy_container(proxmox_host, container_id, cfg=cfg)
        return False
    ...
    verify_commands = [
        ("Check docker version", "docker --version 2>&1"),
        ("Check docker service status", "systemctl is-active docker 2>&1"),
    ]
    ...
    _run_pct_command(
        proxmox_host,
        container_id,
        f"usermod -aG docker {default_user} 2>&1",
        cfg,
    ...
    for action, systemctl_cmd in [
        ("Enable docker service", SystemCtl.enable_cmd("docker")),
        ("Start docker service", SystemCtl.start_cmd("docker")),
    ]:
        succeeded, _ = _run_pct_command(
            proxmox_host,
            container_id,
    ...
    extra_cleanup_steps = [
        ("Stop docker service", SystemCtl.stop_cmd("docker")),
        ("Docker system prune", "docker system prune -af 2>/dev/null || true"),
        (
            "Remove docker install script",
            FileOps.remove_cmd("/tmp/get-docker.sh", force=True),
        ),
    ]
    _cleanup_template_files(
        proxmox_host, container_id, cfg, default_user, extra_steps=extra_cleanup_steps
    )
    ...
    rename_cmd = (
        f"mv '{backup_file}' {template_dir}/{final_template_name} && "
        f"echo 'Template created: {final_template_name}' 2>&1"
    )
    ...
    if not rename_result or "Template created" not in (rename_output or ""):
        error_msg = (
            rename_result.error_type.value
            if rename_result.has_error
            else "No output"
        )
        logger.error("Failed to rename template archive: %s", error_msg)
        # Verify file was actually moved
        verify_cmd = (
            f"test -f {template_dir}/{final_template_name} && "
            f"echo exists || echo missing"
        )
        verify_output = ssh_exec(proxmox_host, verify_cmd, capture_output=True, cfg=cfg)
    ...
    logger.info("Docker template '%s' created successfully", template_name)
    return True
